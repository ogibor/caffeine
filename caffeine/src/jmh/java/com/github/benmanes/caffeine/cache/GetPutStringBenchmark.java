 ...
 * <p>
 * <pre>{@code
 *   ./gradlew jmh -PincludePattern=GetPutStringBenchmark
 * }</pre>
 *
...
public class GetPutStringBenchmark {
  private static final int SIZE = (2 << 18);
  ...
  String repeatedString = "A".repeat(6000);

  BasicCache<Integer, String> cache;
  Integer[] ints;
  ...
  public void setup() {
    ...
    for (int i = 0; i < SIZE; i++) {
      ...
      cache.put(ints[i], repeatedString + i);
    }
  }
  ...
  public String readOnly(ThreadState threadState) {
  ...
  public void writeOnly(ThreadState threadState) {
    cache.put(ints[threadState.index++ & MASK], repeatedString + (threadState.index++ & MASK));
  }

  @Benchmark @Group("readwrite") @GroupThreads(6)
  public String readwrite_get(ThreadState threadState) {
    return cache.get(ints[threadState.index++ & MASK]);
  }
